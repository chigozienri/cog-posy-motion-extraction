...
ffmpeg -i $input_video -filter_complex "[0:v]format=rgb24,split=3[r][g][b];[r]extractplanes=r[red];[g]extractplanes=g[green];[b]extractplanes=b[blue]" -map "[red]" $temp_dir/red.mp4 -map "[green]" $temp_dir/green.mp4 -map "[blue]" $temp_dir/blue.mp4
